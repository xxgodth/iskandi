...
<head>
	...
	<script>
		function show_popup_form(act, lvl_id, lvl_nm)
		{
			...
			level_id.value = (act == "TAMBAH LEVEL PENGGUNA") ? "" : lvl_id;
			
			var level_name = document.getElementById('level_name');
			level_name.value = (act == "TAMBAH LEVEL PENGGUNA") ? "" : lvl_nm;
			level_name.readOnly = (act == "HAPUS LEVEL PENGGUNA") ? true : false;
			
			var btn_submit = document.getElementById('btn_submit');
			btn_submit.value = (act == "HAPUS LEVEL PENGGUNA") ? "Hapus" : "Simpan";
		}
...
<body onload="check_msg()">
	...
	<div id="container">
		<div id="content">
			<h4>Master Level Pengguna</h4>
			<?php if($session[0]->level_id == "1") { ?>
			<span style="float:right; font-size:11px; cursor:pointer;" onclick="show_popup_form('TAMBAH LEVEL PENGGUNA')">
					<i class="fas fa-plus-square green"></i>&nbsp;Tambah Level Pengguna
			</span>
			<?php } ?>
			<p style="color:gray; font-size:12px;">Menampilkan level surat aktif</p>
			<form id="form_search" method="get" action="<?php echo base_url('user_level'); ?>">
				<table id="tbl_search">
					<tr>
						<td>
							Nama Level  Pengguna<br>
				...
				<table id="tbl_data">
					...
					<?php
						...
						else
						{
							...
							{
								$chg = "'UBAH LEVEL PENGGUNA'";
								$del = "'HAPUS LEVEL PENGGUNA'";
			...
			<div style="margin:25px 0 0; text-align:right;">
				<h6 style="font-size:7px;">
					<?php
						if(!empty($size_master_user_level))
						{
							$page = ceil($size_master_user_level/20);
							
							for($i=1; $i<= $page; $i++)
							{
								if(empty($this->input->get('page')) && $i == 1)
								{
									echo '<a href="'.base_url('user_level').'?src='.$src.'&page='.$i.'" class="active_page">'.$i."</a> ";
								}
								...
								{
									echo '<a href="'.base_url('user_level').'?src='.$src.'&page='.$i.'" class="inactive_page">'.$i."</a> ";
								}
							}
						}
						else
	...
	<div id="popup">
		<div id="popup_content">
			...
			<div id="popup_detail">
				<form method="post" action="<?php echo base_url('saving_user_level'); ?>" enctype="multipart/form-data">
					<input type="hidden" class="text" id="act" name="act">
					<input type="hidden" class="text" id="level_id" name="level_id">
					<table id="tbl_popup">
						<tr>
							<td>
								<br>Nama Level Pengguna<br>
								<input type="text" class="text" id="level_name" name="level_name" autocomplete="off" required>
							</td>
						</tr>
						<tr>
							<td>
