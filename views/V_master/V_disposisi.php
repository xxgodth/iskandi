...
<head>
	...
	<script>
		function show_popup_form(act, disp_id, disp_nm)
		{
			...
			var disposisi_id = document.getElementById('disposisi_id');
			disposisi_id.value = (act == "TAMBAH DISPOSISI") ? "" : disp_id;
			
			var disposisi_name = document.getElementById('disposisi_name');
			disposisi_name.value = (act == "TAMBAH DISPOSISI") ? "" : disp_nm;
			disposisi_name.readOnly = (act == "HAPUS DISPOSISI") ? true : false;
			
			var btn_submit = document.getElementById('btn_submit');
			btn_submit.value = (act == "HAPUS DISPOSISI") ? "Hapus" : "Simpan";
		}
...
<body onload="check_msg()">
	...
	<div id="container">
		<div id="content">
			<h4>Master Disposisi</h4>
			<?php if($session[0]->level_id == "1") { ?>
			<span style="float:right; font-size:11px; cursor:pointer;" onclick="show_popup_form('TAMBAH DISPOSISI')">
					<i class="fas fa-plus-square green"></i>&nbsp;Tambah Disposisi
			</span>
			<?php } ?>
			<p style="color:gray; font-size:12px;">Menampilkan disposisi aktif</p>
			<form id="form_search" method="get" action="<?php echo base_url('disposisi'); ?>">
				<table id="tbl_search">
					<tr>
						<td>
							Nama Disposisi<br>
			...
			<div id="div_tbl_data">
				<table id="tbl_data">
					...
					<?php
						...
						{
							foreach($master_disposisi as $rec)
							{
								$chg = "'UBAH DISPOSISI'";
								$del = "'HAPUS DISPOSISI'";
								
								$param = array(
									'disposisi_id' => "'".$rec->disposisi_id."'",
									'disposisi_name' => "'".$rec->disposisi_name."'"
								);
								
								echo '
									<tr>
										<td>'.$no++.'</td>
										<td>'.$rec->disposisi_name.'</td>
										<td>
			...
			<div style="margin:25px 0 0; text-align:right;">
				<h6 style="font-size:7px;">
					<?php
						if(!empty($size_master_disposisi))
						{
							$page = ceil($size_master_disposisi/20);
							
							for($i=1; $i<= $page; $i++)
							{
								if(empty($this->input->get('page')) && $i == 1)
								{
									echo '<a href="'.base_url('disposisi').'?src='.$src.'&page='.$i.'" class="active_page">'.$i."</a> ";
								}
								...
								{
									echo '<a href="'.base_url('disposisi').'?src='.$src.'&page='.$i.'" class="inactive_page">'.$i."</a> ";
								}
							}
						}
						else
	...
	<div id="popup">
		<div id="popup_content">
			...
			<div id="popup_detail">
				<form method="post" action="<?php echo base_url('saving_disposisi'); ?>" enctype="multipart/form-data">
					<input type="hidden" class="text" id="act" name="act">
					<input type="hidden" class="text" id="disposisi_id" name="disposisi_id">
					<table id="tbl_popup">
						<tr>
							<td>
								<br>Nama Disposisi<br>
								<input type="text" class="text" id="disposisi_name" name="disposisi_name" autocomplete="off" required>
							</td>
						</tr>
						<tr>
							<td>
