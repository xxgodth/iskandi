...
<head>
	...
	<script>
		function show_popup_form(act, cmp_id, cmp_nm)
		{
			...
			company_id.value = (act == "TAMBAH INSTANSI") ? "" : cmp_id;
			
			var company_name = document.getElementById('company_name');
			company_name.value = (act == "TAMBAH INSTANSI") ? "" : cmp_nm;
			company_name.readOnly = (act == "HAPUS INSTANSI") ? true : false;
			
			var btn_submit = document.getElementById('btn_submit');
			btn_submit.value = (act == "HAPUS INSTANSI") ? "Hapus" : "Simpan";
		}
...
<body onload="check_msg()">
	...
	<div id="container">
		<div id="content">
			<h4>Master Instansi</h4>
			<?php if($session[0]->level_id == "1") { ?>
			<span style="float:right; font-size:11px; cursor:pointer;" onclick="show_popup_form('TAMBAH INSTANSI')">
					<i class="fas fa-plus-square green"></i>&nbsp;Tambah Instansi Baru
			</span>
			<?php } ?>
			<p style="color:gray; font-size:12px;">Menampilkan instansi aktif</p>
			<form id="form_search" method="get" action="<?php echo base_url('company'); ?>">
				<table id="tbl_search">
					<tr>
						<td>
							Nama Instansi<br>
							<input type="text" name="src" class="text_search" value="<?php echo (!empty($src)) ? $src : ""; ?>" placeholder="Masukkan nama instansi untuk melakukan pencarian" autocomplete="off" autofocus>
						</td>
					</tr>
				</table>
			</form>
			...
			<div style="margin:25px 0 0; text-align:right;">
				<h6 style="font-size:7px;">
					<?php
						if(!empty($size_master_company))
						{
							$page = ceil($size_master_company/20);
	...
	<div id="popup">
		<div id="popup_content">
			...
			<div id="popup_detail">
				<form method="post" action="<?php echo base_url('saving_company'); ?>" enctype="multipart/form-data">
					<input type="hidden" class="text" id="act" name="act">
					<input type="hidden" class="text" id="company_id" name="company_id">
					<table id="tbl_popup">
						<tr>
							<td>
								<br>Nama Instansi<br>
								<input type="text" class="text" id="company_name" name="company_name" autocomplete="off" required>
							</td>
						</tr>
						<tr>
							<td>
